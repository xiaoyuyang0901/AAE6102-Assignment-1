...
if ~exist("navResults.mat", 'file')
    [navResults, ~] = postNavigation(trkResults, settings);
    save("navResults", "navResults");
else
    load("navResults.mat", "navResults");
end
...
% %% 指定结果文件的路径 ===================================================
% % 假设结果文件位于脚本的上一级目录
% % resultsPath = fullfile('..'); % 上一级目录
% resultsPath = 'C:\Users\Administrator\Desktop\GPS_L1CA';
% 
% 
% %% 加载已有结果文件 ====================================================
% % 检查并加载捕获结果
% acqResultsFile = fullfile(resultsPath, 'acqResults.mat');
% if exist(acqResultsFile, 'file')
%     load(acqResultsFile, 'acqResults');
%     disp('   已加载捕获结果（acqResults.mat）...');
% else
%     % 如果文件不存在，则重新运行捕获
%     disp('   捕获结果文件不存在，重新运行捕获...');
%     % Find number of samples per spreading code
%     samplesPerCode = round(settings.samplingFreq / ...
%                        (settings.codeFreqBasis / settings.codeLength));
%     % At least 42ms of signal are needed for fine frequency estimation
...
%     % 执行捕获
%     acqResults = acquisition(data, settings);
%     save(acqResultsFile, 'acqResults');
% end
% 
% %% 初始化通道并准备跟踪 ================================================
% if (any(acqResults.carrFreq))
...
% 
% %% 加载跟踪结果 ==========================================================
% trkResultsFile = fullfile(resultsPath, 'trkResults.mat');
% if exist(trkResultsFile, 'file')
%     load(trkResultsFile, 'trkResults');
%     disp('   已加载跟踪结果（trkResults.mat）...');
% else
%     % 如果文件不存在，则重新运行跟踪
%     disp('   跟踪结果文件不存在，重新运行跟踪...');
%     startTime = now;
%     disp(['   Tracking started at ', datestr(startTime)]);
%     % Process all channels for given data block
%     [trkResults, ~] = tracking(fid, channel, settings);
%     save(trkResultsFile, 'trkResults');
%     fclose(fid);
%     disp(['   Tracking is over (elapsed time ', datestr(now - startTime, 13), ')']);
% end
% 
% %% 加载导航解算结果 =====================================================
% navResultsFile = fullfile(resultsPath, 'navResults.mat');
% if exist(navResultsFile, 'file')
%     load(navResultsFile, 'navResults');
%     disp('   已加载导航解算结果（navResults.mat）...');
% else
%     % 如果文件不存在，则重新运行导航解算
%     disp('   导航解算结果文件不存在，重新运行导航解算...');
%     [navResults, ~] = postNavigation(trkResults, settings);
%     save(navResultsFile, 'navResults');
% end
% 
% %% 绘制结果 ============================================================
% disp('   绘制结果...');
